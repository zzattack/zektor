    ...
    public class SerialPortInfo {
        ...
        private static IEnumerable<SerialPortInfo> GetPortsWMI() {
            ...
            using (var comPortSearcher = new ManagementObjectSearcher(objectQuery)) {
                foreach (string caption in comPortSearcher.Get().Cast<ManagementObject>().Select(obj => obj["Caption"].ToString())
                    .Where(caption => caption.Contains("(COM"))) {
                    string name = caption.Substring(caption.LastIndexOf("(COM", StringComparison.Ordinal)).Replace("(", string.Empty)
        ...
        private static string GetDeviceName(IntPtr pDevInfoSet, SetupApi.SpDevInfoData deviceInfoData) {
            ...
            if (hDeviceRegistryKey == IntPtr.Zero) throw new Exception("Failed to open a registry key for device-specific configuration information");
            ...
            try {
                ...
                if (result != 0) throw new Exception("Can not read registry value PortName for device " + deviceInfoData.ClassGuid);
            }
            finally {
